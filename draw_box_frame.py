...
def main(argv):
    ...
    parser = argparse.ArgumentParser(
        description="undistort and draw box coords on first frame of video"
    )
    ...
    cam_params["distCoeffs"] = np.hstack(
        [cam_params_mat["RDistort"].flatten(), cam_params_mat["TDistort"].flatten()]
    )
    ...
    # undistort
    out_frame = cv2.undistort(
        frame, cam_params["cameraMatrix"], cam_params["distCoeffs"], None, newcameramtx
    )
