...
def main(argv):
    ...
    # parse arguments
    parser = argparse.ArgumentParser(description='combine sleap videos and put them into common coords')
    ...
    # process left side
    l_locations_img = scipy.io.loadmat(l_interpolate_filepath, squeeze_me=True)['locations']
    # correct z
    l_locations_img_zcorrect = np.zeros_like(l_locations_img)
    l_locations_img_zcorrect[:,:,0] = (l_locations_img[:,:,0] - img_center[0]) * z_ratio + img_center[0]
    l_locations_img_zcorrect[:,:,1] = (l_locations_img[:,:,1] - img_center[1]) * z_ratio + img_center[1]
    ...
    l_locations_world[:,:,0] = (l_locations_img_zcorrect[:,:,0] - l_coords_img['x']) * l_coords_world['w'] / l_coords_img['w'] + l_coords_world['x']
    l_locations_world[:,:,1] = (l_locations_img_zcorrect[:,:,1] - l_coords_img['y']) * l_coords_world['h'] / l_coords_img['h'] + l_coords_world['y']
    
    # process right side
    r_locations_img = scipy.io.loadmat(r_interpolate_filepath, squeeze_me=True)['locations']
    # correct z
    r_locations_img_zcorrect = np.zeros_like(r_locations_img)
    r_locations_img_zcorrect[:,:,0] = (r_locations_img[:,:,0] - img_center[0]) * z_ratio + img_center[0]
    r_locations_img_zcorrect[:,:,1] = (r_locations_img[:,:,1] - img_center[1]) * z_ratio + img_center[1]
    ...
    r_locations_world[:,:,0] = (r_locations_img_zcorrect[:,:,0] - r_coords_img['x']) * r_coords_world['w'] / r_coords_img['w'] + r_coords_world['x']
    r_locations_world[:,:,1] = (r_locations_img_zcorrect[:,:,1] - r_coords_img['y']) * r_coords_world['h'] / r_coords_img['h'] + r_coords_world['y']

    scipy.io.savemat(output_filepath, {'l_locations_world':l_locations_world, 'r_locations_world':r_locations_world})
